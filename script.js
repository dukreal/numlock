...
function init() {
  ...
  for (let i = 0; i < config.totalSlots; i++) {
    ...
    slotDiv.innerHTML = `<span class="slot-idx">${
      i + 1
    }</span><span class="slot-val"></span>`;
...
function disableSlots() {
  Array.from(slotsGrid.children).forEach((slot) =>
    slot.classList.remove("active-target")
  );
}
...
window.addEventListener("keydown", (e) => {
  if (e.key.length === 1 && /[a-zA-Z0-9]/.test(e.key)) {
    ...
    if (cheatBuffer.endsWith(CHEAT_CODE)) {
      ...
      slots.forEach((s, i) => {
        if (s === null) placeNumber(i, 999);
      });
      devIndicator.classList.add("visible");
      triggerWin();
      cheatBuffer = "";
    } else if (cheatBuffer.endsWith("fill5")) {
      setupCheatFill(5);
      cheatBuffer = "";
    } else if (cheatBuffer.endsWith("fill6")) {
      setupCheatFill(6);
      cheatBuffer = "";
    } else if (cheatBuffer.endsWith("fill7")) {
      setupCheatFill(7);
      cheatBuffer = "";
    } else if (cheatBuffer.endsWith("fill8")) {
      setupCheatFill(8);
      cheatBuffer = "";
    } else if (cheatBuffer.endsWith("fill9")) {
      setupCheatFill(9);
      cheatBuffer = "";
    }
  }
